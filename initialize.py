...
def batched_k_local(dirac_point, radius=0.2, Lx=21, Ly=21):
    """
    ...
    Args:
        ...
        Lx, Ly: Number of k-points in each direction
        
    Returns:
        k_points: Array of shape (Lx*Ly, 2) containing k-points
    """
    # Create local grid centered at Dirac point
    kx_center, ky_center = dirac_point
    
    # Generate local grid with uniform spacing
    kx_local = jnp.linspace(kx_center - radius, kx_center + radius, Lx)
    ky_local = jnp.linspace(ky_center - radius, ky_center + radius, Ly)
    
    X, Y = jnp.meshgrid(kx_local, ky_local)
    return jnp.array([X.flatten(), Y.flatten()]).T

def batched_Gin_local(dirac_point, radius, Lx, Ly, Nv):
    ...
    return vmap(lambda k: _gamma_in(k, Nv), 0)(batched_k_local(dirac_point, radius, Lx, Ly))

# ============================================================================
# Local Correlator and Loss Functions
# ============================================================================

def make_correlator_local(dirac_point, radius, Lx, Ly, Nv):
    """Create correlator function for local momentum region."""
    Df = 2 * NF  # 2 for Kitaev
    Gin = batched_Gin_local(dirac_point, radius, Lx, Ly, Nv)
...
def make_loss_local(dirac_point, radius, Lx, Ly, Nv, Jx=1.0, Jy=1.0, Jz=1.0):
    """Create energy loss function for local momentum region."""
    # Prepare local momentum points and energy kernels
    batch_k = batched_k_local(dirac_point, radius, Lx, Ly)
    batch_h = vmap(lambda k: kitaev_kernel(k, Jx=Jx, Jy=Jy, Jz=Jz))(batch_k)
    correlator = make_correlator_local(dirac_point, radius, Lx, Ly, Nv)
...
class KitaevfPEPSLocal(KitaevfPEPS):
    ...
    def __init__(self, Lx, Ly, Nv, dirac_point=(jnp.pi/3, -jnp.pi/3), 
                 radius=0.2, Jx=1.0, Jy=1.0, Jz=1.0, seed=42, T=None):
        ...
        self.loss = make_loss_local(dirac_point, radius, Lx, Ly, Nv, Jx, Jy, Jz)
...
def run_local_initialization(config=None):
    ...
    local_config = {
        ...
        "local_Lx": 101,                         # Local grid points in x
        "local_Ly": 101,                         # Local grid points in y
    ...
    local_Lx, local_Ly = config["local_Lx"], config["local_Ly"]
    Nv = config.get("Nv", 2)
...
def run_auto_analysis(saved_files_info, dirac_point, analysis_radius=0.001, 
    ...
    Args:
        ...
        analysis_radius: Analysis radius around Dirac point
    ...
    print(f"   Analysis radius: {analysis_radius}")
    print(f"   Rings: {n_rings}, Angles per ring: {n_angles}")
...
def parse_args():
    ...
    parser.add_argument("--local_Lx", type=int, default=51,
                       help="Number of k-points in x direction (local)")
    parser.add_argument("--local_Ly", type=int, default=51,
                       help="Number of k-points in y direction (local)")
    ...
    parser.add_argument("--analysis_radius", type=float, default=None,
                       help="Analysis radius around Dirac point for dense analysis")
    parser.add_argument("--n_rings", type=int, default=10,
                       help="Number of concentric rings for dense analysis")
    parser.add_argument("--n_angles", type=int, default=32,
                       help="Number of angular points per ring for dense analysis")
